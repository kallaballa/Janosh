...
static std::vector<kt::TimedDB*> proc(const std::vector<std::string>& dbpaths,
  ...
  Worker* worker = new Worker(thnum, condmap, dbs[0], dbnum, dbmap, omode, asi, ash, bgspath, bgsi, bgscomp,
...
//    g_restart = false;
    ...
    serverThread = new std::thread([=](){
      if (g_serv->start()) {
        ...
        g_serv->log(Logger::ERROR, "Unable to start RPCServer");
      }
    });
    return dbs;
}
